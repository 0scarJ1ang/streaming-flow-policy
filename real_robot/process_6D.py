...
class DataProcessorStatic:
    ...
    def process(self, image_path, image_time_path, ee_pose_path, gripper_path, cam_pose_path, output_csv):
        ...
        # )
        static_obj_pos = [0.48072949605208243,-0.04797794031182588,0.1094363041528675]
        ...
        for t_robot in ee_times:
            ...
            pose_mat = torch.tensor(R.from_quat(ee_quat).as_matrix())
            pose_6d = list(map(float,matrix_to_rotation_6d(torch.tensor(pose_mat).unsqueeze(0))[0]))
            ...
            row = {
                ...
                'pose_6d_0': pose_6d[0], 'pose_6d_1': pose_6d[1], 'pose_6d_2': pose_6d[2], 'pose_6d_3': pose_6d[3], 'pose_6d_4': pose_6d[4], 'pose_6d_5': pose_6d[5],
                'gripper': g_val,
...
if __name__ == "__main__":
    ...
    for trial_id in range(total_trials):
        ...
        try:
            processor.process(
                image_path=f"datasets/item_grasping_s2/images_{trial_id}.npy",
                image_time_path=f"datasets/item_grasping_s2/image_time_{trial_id}.npy",
                ee_pose_path=f"datasets/item_grasping_s2/ee_pose_{trial_id}.json",
                gripper_path=f"datasets/item_grasping_s2/gripper_{trial_id}.json",
                cam_pose_path=f"datasets/item_grasping_s2/cam_pose_{trial_id}.json",  # 这个文件是所有 trial 共用的
                output_csv=f"{output_dir}/train_data_trial_2_{trial_id}.csv"
            )
