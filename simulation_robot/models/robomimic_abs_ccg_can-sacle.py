...
try:
    ...
    dataset_path = "/home/users/meiyi/streaming-flow-policy/notebooks/pusht/Robotic_mimic/robomimic/datasets/can/ph/low_dim_abs.hdf5"
...
def test():
    ...
    dataset_path = '/home/cchi/dev/diffusion_policy/data/robomimic/datasets/can/ph/low_dim.hdf5'
...
# 经验对齐：根据数据集 action[:3] 的统计判断它更像 delta 还是 absolute
# - delta: 均值接近 0（围绕 0 抖动）
# - absolute: 均值通常偏离 0（工作空间坐标）
_inferred_control_delta = None
try:
    with h5py.File(dataset_path, 'r') as f:
        a = f['data/demo_0/actions'][:]
        a_xyz = a[: min(2000, a.shape[0]), :3].astype(np.float32)
        mean_abs = float(np.mean(np.abs(a_xyz)))
        mean = a_xyz.mean(axis=0)
        std = a_xyz.std(axis=0)
        print(f"[dataset action stats] mean_abs(|xyz|)={mean_abs:.4f} mean={mean} std={std}")
        _inferred_control_delta = bool(mean_abs < 0.10)
except Exception as e:
    print(f"[warn] failed to infer control_delta from dataset actions: {e}")

# 最终策略：优先相信元数据；只有元数据缺失时才用统计推断
CONTROL_DELTA = ccfg.get('control_delta', None)
if CONTROL_DELTA is None and _inferred_control_delta is not None:
    CONTROL_DELTA = _inferred_control_delta

# 先打印“初始猜测”（后面会用 demo 回放做一次更可靠的自校准）
print(f"[control_delta] initial CONTROL_DELTA (from meta/stats) = {CONTROL_DELTA}")
...
# ----------------------------------------------------------
# [关键自校准] 通过回放数据集 demo_0 的原始 action，判断 control_delta=True/False 谁能复现成功奖励
# 这比“均值/方差猜”靠谱得多；can / square 这类任务经常出现元数据不一致的问题
# ----------------------------------------------------------
def _infer_control_delta_by_replay(dataset_path: str, env_meta: dict, demo_key: str = "demo_0") -> bool:
    import h5py
    import numpy as np
    import robomimic.utils.env_utils as EnvUtils
    ...
    def _rollout(cd: bool) -> float:
        # clone env_meta
        meta = dict(env_meta)
        env_kwargs = dict(meta.get("env_kwargs", {}))
        ccfg = dict(env_kwargs.get("controller_configs", {}))
        ccfg["control_delta"] = bool(cd)
        env_kwargs["controller_configs"] = ccfg
        meta["env_kwargs"] = env_kwargs
    ...
    max_r_true = _rollout(True)
    max_r_false = _rollout(False)
    best = False if (max_r_false >= max_r_true) else True
    print(f"[control_delta replay] max_r(True)={max_r_true} | max_r(False)={max_r_false} -> choose {best}")
    return bool(best)

try:
    _replay_cd = _infer_control_delta_by_replay(dataset_path=dataset_path, env_meta=env_meta, demo_key="demo_0")
    # 以回放为准覆盖
    CONTROL_DELTA = bool(_replay_cd)
except Exception as e:
    print(f"[warn] control_delta replay inference failed, keep initial CONTROL_DELTA={CONTROL_DELTA}. err={e}")

# 统一覆盖到 env_meta，确保 env/控制器配置与推理逻辑一致
env_meta["env_kwargs"]["controller_configs"]["control_delta"] = bool(CONTROL_DELTA)
print(f"[env_meta] OVERRIDE: controller_configs.control_delta = {env_meta['env_kwargs']['controller_configs']['control_delta']}")
print(f"[control_delta] final CONTROL_DELTA = {CONTROL_DELTA}")
...
wrapper = RobomimicLowdimWrapper(
    ...
    render_hw=(256, 256),
    render_camera_name='agentview',
    # delta 控制时需要严格裁剪到 [-1,1]；absolute 控制时不裁剪避免把 workspace 坐标截断
    clip_action=bool(CONTROL_DELTA) if CONTROL_DELTA is not None else True
)
...
# resume_from = "/home/users/meiyi/streaming-flow-policy/notebooks/pusht/Robotic_mimic/models/can_state_800_ep_si_abs.ckpt"
...
#             ckpt_path_epoch = os.path.join(save_dir, f"can_state_{epoch_idx+1}_ep_si_abs.ckpt")
#             ckpt_path_latest = os.path.join(save_dir, "can_state_latest_si_abs.ckpt")
...
# # 保存模型
# ckpt_path_si = os.path.join(save_dir, f"can_state_{num_epochs_si}_ep_si_abs.ckpt")
# ckpt_path_latest = os.path.join(save_dir, "can_state_latest_si_abs.ckpt")
...
ckpt_path_si = "/home/users/meiyi/streaming-flow-policy/notebooks/pusht/Robotic_mimic/models/can_state_1200_ep_si_abs.ckpt"
...
# =========================================================
# 核心修复: SSIP 推理函数（自动适配 control_delta：delta / absolute）
# =========================================================
def run_si_inference(sigma_infer, title):
    ...
    # --- 修改建议 ---
    # 动态找到 'robot0_eef_pos' 在扁平向量中的起始位置
    # 这是一个更通用的写法，以后换任务不用改这里
    ...
    # 当前观测里的 eef 位置（物理坐标）
    curr_pos = obs[start_dim : start_dim+3].astype(np.float32) # (x, y, z)

    # helper：把 raw action 映射到 action_normalizer 的归一化空间 [-1, 1]
    def _raw_to_norm(raw: np.ndarray, min_v: np.ndarray, max_v: np.ndarray) -> np.ndarray:
        raw = raw.astype(np.float32)
        min_v = min_v.astype(np.float32)
        max_v = max_v.astype(np.float32)
    ...
    if is_delta:
        # delta 控制：初始 latent action 应该是 “0 增量”
        raw_pos = np.zeros(3, dtype=np.float32)
    else:
        # absolute 控制：初始 latent action 用当前 eef_pos 更合理
        raw_pos = curr_pos
    ...
    norm_pos = _raw_to_norm(raw_pos, pos_min, pos_max)
    
    # 3. 初始化 na (Latent State)
    # 维度: (1, 1, 10)
    na = torch.zeros((1, 1, 10), device=device, dtype=torch.float32)
    
    # [Fix 1]: 覆盖位置（根据 control_delta 自动选择 “0增量” 或 “当前eef_pos”）
    ...
    # 对于 delta/absolute 两种控制，“零旋转/单位旋转” 都应对应同一个稳定 6D 表示
    ...
    rot6d_norm = _raw_to_norm(rot6d_raw, rot_min, rot_max)
    na[0, 0, 3:9] = torch.from_numpy(rot6d_norm).to(device, dtype=torch.float32)
        ...
        with tqdm(total=max_steps, desc=title) as pbar:
            while not done and step_idx < max_steps:
                ...
                for i in range(action_horizon):
                    ...
                    # success 判定（强鲁棒）：reward>0 或 info success 标记
                    ...
                    # 数值安全：限制 latent 在 [-1, 1]，并清理 NaN/Inf
...
def run_ccg_phys_vis_inference(
    ...
    render=True,              # 是否渲染（生成 imgs，用于可视化/GIF）
):
    # =========================================================
    ...
    obs = wrapper.reset()
    obs_deque = collections.deque([obs] * obs_horizon, maxlen=obs_horizon)
    # 渲染开关：关闭时避免任何 render 调用，imgs 保持为空
    imgs = [wrapper.render()] if (render and (not collect_trajectory)) else []
    rewards = []
    ...
    with torch.no_grad():
        ...
        while not done and step_idx < max_steps:
            ...
            for i in range(action_horizon):
                ...
                if (not collect_trajectory) and render:
...
def run_experiment_100_trials(render_and_save=True):
    ...
    # 1. 建立文件夹结构（可选：仅在 render_and_save=True 时创建）
    ...
    while valid_count < total_needed:
        ...
        imgs, max_r_ccg, _, has_collided = run_ccg_phys_vis_inference(
            ...
            activation_dist=0.2,
            seed=current_seed,
            collect_trajectory=False, # 是否收集轨迹
            render=render_and_save,   # 是否渲染（用于 GIF）
        )
    ...
    if render_and_save:
        ...
        print("本次未开启渲染/保存（render_and_save=False），未生成 GIF 文件。")

# 运行实验
# 开关：是否渲染并保存 GIF
RENDER_AND_SAVE = False
run_experiment_100_trials(render_and_save=RENDER_AND_SAVE)

"""
CUDA_VISIBLE_DEVICES=2 nohup python -u /home/users/meiyi/streaming-flow-policy/notebooks/pusht/Robotic_mimic/robomimic_abs_ccg_can-sacle.py > ccg_can_eval.log 2>&1 &
"""